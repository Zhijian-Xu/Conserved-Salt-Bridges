...
results/step02_saltbridges_raw/<family_id>.txt
...
user-defined (recommended): results/step03_clusters/<family_id>.txt
...
results/step04_ca_distances/{all,cla,nocla,other,error}/<family_id>.txt
...
results/step04_ca_distances/*/<family_id>.txt
...
results/step07_true_conserved_k5/<family_id>.txt

Citation
If you use this pipeline in academic work, please cite our manuscript and/or this repository (recommended: add CITATION.cff).
...
## 你接下来只需要做 2 件小事（否则 Step02 仍可能跑不通）

1) 把 `com.py` / `com2.py` 放进 repo，例如：`scripts/utils/com.py`、`scripts/utils/com2.py`  
   因为你的 Step02 检测盐桥要用它们（计算“中心”）。
...
如果你愿意，我也可以顺手帮你把 `step01/step02` 再做一层增强：  
- 支持 `.cif`（非 gz）  
- 支持并行（按家族并行）  
- 输出更规范（比如同时输出 CSV）
